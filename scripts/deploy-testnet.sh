...
# ── 5b. Deploy + initialize yield_nft, seed its reserve ──────────────────────
NFT_WASM="contracts/target/wasm32v1-none/release/conduit_yield_nft.wasm"
[ -f "$NFT_WASM" ] || { echo "ERROR: wasm not found at $NFT_WASM" >&2; exit 1; }
say "Deploying yield_nft"
NFT_ID="$(stellar contract deploy --wasm "$NFT_WASM" \
  --source "$IDENTITY" "${NET[@]}")"
ok "Deployed yield_nft: $NFT_ID"

stellar contract invoke --id "$NFT_ID" \
  --source "$IDENTITY" "${NET[@]}" -- \
  initialize --admin "$ADMIN" --token "$TOKEN_ID" --oracle "$ORACLE_ID"
ok "yield_nft initialized"

say "Seeding yield_nft reserve with ${RESERVE_XLM} XLM (covers NFT yield payouts)"
stellar contract invoke --id "$TOKEN_ID" \
  --source "$IDENTITY" "${NET[@]}" -- \
  transfer --from "$ADMIN" --to "$NFT_ID" --amount "$RESERVE_STROOPS"
ok "yield_nft reserve seeded"

# ── 5c. Deploy + initialize cond_executor ────────────────────────────────────
COND_WASM="contracts/target/wasm32v1-none/release/conduit_cond_executor.wasm"
[ -f "$COND_WASM" ] || { echo "ERROR: wasm not found at $COND_WASM" >&2; exit 1; }
say "Deploying cond_executor"
COND_ID="$(stellar contract deploy --wasm "$COND_WASM" \
  --source "$IDENTITY" "${NET[@]}")"
ok "Deployed cond_executor: $COND_ID"

stellar contract invoke --id "$COND_ID" \
  --source "$IDENTITY" "${NET[@]}" -- \
  initialize --admin "$ADMIN" --router "$CONTRACT_ID" --oracle "$ORACLE_ID"
ok "cond_executor initialized (router + oracle wired)"
...
YIELD_NFT_CONTRACT_ID=$NFT_ID
COND_EXECUTOR_CONTRACT_ID=$COND_ID
...
echo "  yield_nft     : $NFT_ID"
echo "  cond_executor : $COND_ID"
