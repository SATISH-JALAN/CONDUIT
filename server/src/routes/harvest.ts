...
import { harvests, positions } from '../db/schema.js';
...
app.post('/build', async (c) => {
  try {
    ...
    // Build unsigned XDR
    const { xdr, networkPassphrase } = await buildHarvestTx(wallet, pendingYield, box_id);
    ...
    return c.json({ error: err.message || 'Failed to build harvest transaction' }, 500);
  }
});
...
app.post('/submit', async (c) => {
  try {
    ...
    try {
      ...
      logger.error({ err: stellarErr.message }, 'Stellar harvest submit failed');
      return c.json({ error: 'Transaction rejected: ' + stellarErr.message }, 400);
    }
