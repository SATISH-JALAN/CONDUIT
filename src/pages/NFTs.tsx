    ...
    <AppLayout>
      <div className="max-w-[1200px] mx-auto" ref={containerRef}>
        <header className="nft-item mb-8 md:mb-10">
          ...
          <p className="mt-3 max-w-[740px] font-secondary text-[15px] text-[var(--ink-3)] leading-[1.7]">
        ...
        <section className="grid sm:grid-cols-2 xl:grid-cols-4 gap-6">
          {NFT_YIELDS.map((item) => (
            <article
              key={item.id}
              className="nft-item paper-card p-6 border-t-[2px] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_24px_rgba(0,0,0,0.06)]"
              style={{ borderTopColor: `var(${item.accent.replace('var(', '').replace(')', '')})` }}
            >
              <div className="flex items-start justify-between mb-6">
                <div>
                  <div className="text-mono text-[10px] uppercase tracking-[0.1em] text-[var(--ink-4)] mb-1">{item.box}</div>
                  <h3 className="font-display text-[20px] tracking-[-0.02em] text-[var(--ink-1)]">{item.name}</h3>
                </div>
                <Sparkles size={15} className="text-[var(--ink-4)]" />
              </div>

              <div className="space-y-4">
                <div>
                  <div className="text-mono text-[10px] uppercase text-[var(--ink-4)]">Yield Rights</div>
                ...
                <div className="pt-4 border-t border-[var(--paper-edge)] flex items-center justify-between">
                  <div>
                    <div className="text-mono text-[10px] uppercase text-[var(--ink-4)]">Floor</div>
                    <div className="font-secondary text-[15px] text-[var(--ink-1)]">{item.floor}</div>
                  </div>
                  <button className="neo-btn px-3 py-2 text-[12px] font-display text-[var(--ink-2)] inline-flex items-center gap-1">
