...
export const assetTypeEnum = pgEnum('asset_type', ['Government', 'Corporate', 'Sovereign']);
export const riskToleranceEnum = pgEnum('risk_tolerance', ['Conservative', 'Moderate', 'Aggressive']);
export const creditRatingEnum = pgEnum('credit_rating', ['AAA', 'AA', 'A', 'BBB']);
...
export const positions = pgTable('positions', {
  ...
  wallet: text('wallet').notNull().references(() => users.wallet),
  boxId: text('box_id').notNull().references(() => bondBoxes.id),
...
export const mandates = pgTable('mandates', {
  wallet: text('wallet').primaryKey().references(() => users.wallet),
  ...
  compoundThresholdCents: integer('compound_threshold_cents').default(5000).notNull(),
